...
#define OBJECT_TYPE_MYSTERIOUS 4
#define OBJECT_TYPE_COIN	5
#define OBJECT_TYPE_BACKGROUND 12

#define INDES_TYPE_FLOOR 1
#define INDES_TYPE_PIPE 2
...
void CPlayScene::_ParseSection_INDESTRUCTIBLES(string line)
{
	vector<string> tokens = split(line);
	if (tokens.size() < 4) return;
	int Indes_type = atoi(tokens[0].c_str());
	float l = atof(tokens[1].c_str());
	float t = atof(tokens[2].c_str());
	float r = atof(tokens[3].c_str());
	float b = atof(tokens[4].c_str());
	
	if (Indes_type == INDES_TYPE_PIPE && r - l > 32)
	{
		DebugOut(L"Pipe too WIDEEEEEEEEEEEEEEE");
		return;
	}
	if (Indes_type != 3)
	{
		CGameObject* obj = NULL;
		obj = new CINDES(l, t, r, b);
		obj->SetPosition(l, t);
		objects.push_back(obj);
	}
	else
	{
		CGameObject* obj = new Platform(l,t,r,b);
		obj->SetPosition(l, t);
		objects.push_back(obj);
...
void CPlayScene::_ParseSection_OBJECTS(string line)
{
	...
	case OBJECT_TYPE_MYSTERIOUS: obj = new CMysterious(); break;
...
void CPlayScene::Load()
{
	...
	while (f.getline(str, MAX_SCENE_LINE))
	{
		...
		if (line == "[INDESTRUCTIBLES]")
		{
			section = SCENE_SECTION_INDESTRUCTIBLES; continue;
		}
		...
		{ 
			...
			case SCENE_SECTION_INDESTRUCTIBLES: _ParseSection_INDESTRUCTIBLES(line); break;
...
CPlayScene* CPlayScene::getScene()
{
...
void CPlayScenceKeyHandler::OnKeyUp(int KeyCode)
{
	...
	{
	case DIK_C:
		mario->SetState(MARIO_NORMALIZE);
		break;
	case DIK_SPACE:
